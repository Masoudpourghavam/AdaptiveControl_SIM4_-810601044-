...
% Question 3-2 Indirect adaptive J3 with disturbance
...
%         [Uc(i-1) Uc(i-2)]*betaPrime(1:end-1)')/Ac(1); % + e(i)
    ...
    if d == 1
          u(i) = CONSTANT*(-u(i-2)*betaPrime + Predicted_y -...
              [y(i) y(i-1)]*alpha'+lambda*P_prime*[u_hat(i-1) u_hat(i-2)]'-lambda*R_prime*[u(i-1) u(i-2)]')+ Disturbance(i); % + Disturbance(i)
    ...
    elseif d == 2
        ...
        u(i) = CONSTANT*(-[u(i-1) u(i-2)]*betaPrime' + Predicted_y - [y(i) y(i-1)]*alpha'+lambda*P_prime*[u_hat(i-1) u_hat(i-2)]'-lambda*R_prime*[u(i-1) u(i-2)]')+ Disturbance(i); % + Disturbance(i)
        u_hat(i) = -P_prime*[u_hat(i-1) u_hat(i-2)]'+R_prime*[u(i-1) u(i-2)]'; 
            if u(i) <= Ub_u && u(i) >= Lb_u
                u(i) = u(i);
    ...
    elseif d == 3
        ...
        u(i) = CONSTANT*(-[u(i-1) u(i-2) u(i-3)]*betaPrime'...
            + Predicted_y - [y(i) y(i-1)]*alpha'-lambda*(-P_prime*[u_hat(i-1) u_hat(i-2)]'+R_prime*[u(i-1) u(i-2)]'))+ Disturbance(i); % + Disturbance(i)
        u_hat(i) = -P_prime*[u_hat(i-1) u_hat(i-2)]'+R_prime*[u(i-1) u(i-2)]'; 
            if u(i) <= Ub_u && u(i) >= Lb_u
                u(i) = u(i);
