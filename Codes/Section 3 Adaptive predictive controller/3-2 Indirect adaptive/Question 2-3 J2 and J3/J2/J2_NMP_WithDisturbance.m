...
% Question 3-2 Indirect adaptive J2 With disturbance
...
%         [Uc(i-1) Uc(i-2)]*betaPrime(1:end-1)')/Ac(1); % + e(i)
    ...
    if d == 1
        u(i) = CONSTANT*(-u(i-2)*betaPrime + Predicted_y -...
            [y(i) y(i-1)]*alpha')+ Disturbance(i); % + Disturbance(i)
            % saturation
    ...
    elseif d == 2
        ...
        u(1,i) = CONSTANT*(-[u(i-1) u(i-2)]*betaPrime' + Predicted_y - [y(i) y(i-1)]*alpha')+ Disturbance(i); % + Disturbance(i)
            if u(i) <= Ub_u && u(i) >= Lb_u
                u(i) = u(i);
    ...
    elseif d == 3
        ...
        u(i) = CONSTANT*(-[u(i-1) u(i-2) u(i-3)]*betaPrime'...
            + Predicted_y - [y(i) y(i-1)]*alpha')+ Disturbance(i); % + Disturbance(i)
            if u(i) <= Ub_u && u(i) >= Lb_u
                u(i) = u(i);
        ...
        u(i) = CONSTANT*(-[u(i-1) u(i-2) u(i-3) u(i-4)]*betaPrime'...
            + Predicted_y - [y(i) y(i-1)]*alpha')+ Disturbance(i); % + DisturbDisturbance(i)
            if u(i) <= Ub_u && u(i) >= Lb_u
                u(i) = u(i);
